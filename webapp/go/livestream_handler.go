...
func reserveLivestreamHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &slots, "SELECT * FROM reservation_slots WHERE start_at >= ? AND start_at < ? FOR UPDATE", req.StartAt, req.EndAt); err != nil {
	...
	if _, err := tx.ExecContext(ctx, "UPDATE reservation_slots SET slot = slot - 1 WHERE start_at >= ? AND start_at < ?", req.StartAt, req.EndAt); err != nil {
