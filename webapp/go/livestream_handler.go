...
func searchLivestreamsHandler(c echo.Context) error {
	...
	if c.QueryParam("tag") != "" {
		...
		livestreamIDList := []int64{}
		for _, keyTaggedLivestream := range keyTaggedLivestreams {
			livestreamIDList = append(livestreamIDList, keyTaggedLivestream.LivestreamID)
		}
		if len(livestreamIDList) > 0 {
			query, params, err := sqlx.In("SELECT * FROM livestreams WHERE id IN (?) ORDER BY id DESC", livestreamIDList)
			if err != nil {
				return echo.NewHTTPError(http.StatusInternalServerError, "failed to construct IN query: "+err.Error())
			}
...
func getLivestreamTags(ctx context.Context, tx *sqlx.Tx, livestream_id int64) ([]Tag, error) {
	/*
	...
	*/
	...
	var tags = make([]Tag, 0)
	if err := tx.SelectContext(ctx, &tags, "SELECT t.* FROM tags t JOIN livestream_tags lt ON lt.tag_id = t.id WHERE lt.livestream_id = ?", livestream_id); err != nil {
		return tags, err
	}
	/*
